...
function Card({ children, style = {} }) {
  return (
    <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:16, padding:'1.5rem', boxShadow:'0 1px 6px rgba(0,0,0,0.05)', ...style }}>
      {children}
    </div>
  );
}
...
function Label({ children }) {
  return <label style={{ display:'block', fontSize:'0.8125rem', fontWeight:600, color:NAVY, marginBottom:'0.375rem', fontFamily:"'DM Sans',sans-serif" }}>{children}</label>;
}
  ...
  return (
    <div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:'1rem' }}>
      ...
      <div>
        ...
        {subtitle && <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', color:'#6b7280', lineHeight:1.5, marginTop:2 }}>{subtitle}</div>}
      </div>
    </div>
  );
}
...
function Divider() {
  return <div style={{ borderTop:'1.5px solid #f3f4f6', margin:'1.75rem 0' }}/>;
}
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'0.625rem', marginBottom:'1.25rem' }}>
        <Stat label="Total Contributions" value={fmtPlain(contributed)} color='#6b7280'/>
        <Stat label="Interest Earned" value={fmtPlain(growth)} color='#22c55e'/>
        <Stat label="Final Balance" value={fmt(total)} color={TEAL} size="lg"/>
      </div>
      {milestones.length > 0 && (
        <>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:NAVY, marginBottom:'0.625rem' }}>Balance at milestones</div>
          <div style={{ display:'grid', gridTemplateColumns:`repeat(${milestones.length},1fr)`, gap:'0.5rem' }}>
            {milestones.map(y => (
              <div key={y} style={{ background:`${TEAL}08`, border:`1px solid ${TEAL}20`, borderRadius:10, padding:'0.625rem 0.5rem', textAlign:'center' }}>
                ...
                <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.75rem', color:'#9ca3af', marginTop:2 }}>Year {y}</div>
              </div>
            ))}
          </div>
        </>
...
function InflationCalc() {
  ...
  return (
    <div>
      ...
      <div style={{ background:'#fffbeb', border:'1px solid #fde68a', borderRadius:10, padding:'0.75rem 0.875rem', fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', color:'#374151', lineHeight:1.7 }}>
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 1.5rem' }}>
        <div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:'#22c55e', marginBottom:'0.75rem', textTransform:'uppercase', letterSpacing:'0.04em' }}>Assets</div>
          {assetFields.map(f => (
            <NumInput key={f.key} label={f.label} value={assets[f.key]} onChange={v => setAssets(a=>({...a,[f.key]:v}))} step={500}/>
          ))}
        </div>
        <div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:'#ef4444', marginBottom:'0.75rem', textTransform:'uppercase', letterSpacing:'0.04em' }}>Liabilities</div>
          {liabFields.map(f => (
      ...
      <div style={{ marginTop:'1rem', fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', color:'#6b7280', lineHeight:1.65 }}>
        {netWorth < 0
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 1.25rem' }}>
        ...
        <div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:NAVY, marginBottom:'0.75rem' }}>Your Savings</div>
          <div style={{ marginBottom:'1rem' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'0.625rem', marginBottom:'1.25rem' }}>
        <Stat label="Monthly Essentials" value={fmtPlain(essential)} color='#6b7280'/>
  ...
  return (
    <div>
        ...
        <div style={{ marginBottom:'1rem' }}>
          ...
          <div style={{ display:'flex', gap:'0.5rem' }}>
            {[{id:'single',label:'Single'},{id:'mfj',label:'Married Filing Jointly'}].map(s => (
              <button key={s.id} onClick={() => setStatus(s.id)} style={{ flex:1, padding:'9px 8px', borderRadius:9, border:`1.5px solid ${status===s.id?'#8b5cf6':'#e5e7eb'}`, background: status===s.id?'#8b5cf6':'#fafafa', color: status===s.id?'#fff':NAVY, fontFamily:"'DM Sans',sans-serif", fontSize:'0.8rem', fontWeight:700, cursor:'pointer' }}>{s.label}</button>
            ))}
          </div>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'0.625rem', marginBottom:'1.25rem' }}>
        <Stat label="Taxable Income" value={fmtPlain(taxable)} color='#6b7280'/>
      ...
      <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:NAVY, marginBottom:'0.625rem' }}>Tax by bracket</div>
      <div style={{ display:'flex', flexDirection:'column', gap:'0.375rem', marginBottom:'1rem' }}>
        {breakdown.map((b, i) => (
          <div key={i} style={{ display:'flex', alignItems:'center', gap:10 }}>
            <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8rem', color:'#6b7280', minWidth:32 }}>{(b.rate*100).toFixed(0)}%</span>
            ...
            <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8rem', fontWeight:600, color:NAVY, minWidth:70, textAlign:'right' }}>{fmtPlain(b.tax)}</span>
          </div>
        ))}
      </div>

      <div style={{ background:`${TEAL}0d`, border:`1px solid ${TEAL}22`, borderRadius:12, padding:'0.875rem 1rem', fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', color:'#374151', lineHeight:1.7 }}>
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.75rem', marginBottom:'1.25rem' }}>
        <div style={{ background:'#8b5cf615', border:'1.5px solid #8b5cf630', borderRadius:12, padding:'1rem', textAlign:'center' }}>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:'#8b5cf6', marginBottom:6 }}>Traditional IRA</div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.75rem', color:'#6b7280', marginBottom:4 }}>After-tax value at withdrawal</div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.75rem', fontWeight:700, color:'#8b5cf6' }}>{fmt(tradAfterFull)}</div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.75rem', color:'#9ca3af', marginTop:4 }}>Gross: {fmt(tradFVFull)} — taxed at {retireRate}%</div>
        </div>
        <div style={{ background:'#22c55e15', border:'1.5px solid #22c55e30', borderRadius:12, padding:'1rem', textAlign:'center' }}>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight:700, color:'#22c55e', marginBottom:6 }}>Roth IRA</div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.75rem', color:'#6b7280', marginBottom:4 }}>Tax-free value at withdrawal</div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.75rem', fontWeight:700, color:'#22c55e' }}>{fmt(rothFVFull)}</div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.75rem', color:'#9ca3af', marginTop:4 }}>No taxes owed at withdrawal</div>
        </div>
      </div>

      <div style={{ background: (rothWins ? '#22c55e' : '#8b5cf6') + '0d', border:`1.5px solid ${rothWins ? '#22c55e' : '#8b5cf6'}30`, borderRadius:12, padding:'1rem', fontFamily:"'DM Sans',sans-serif" }}>
  ...
  return (
    <div style={{ minHeight:'100vh', background:BG, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ background:NAVY, padding:'2rem 2.5rem 1.5rem' }}>
        ...
        <p style={{ margin:0, fontSize:'1rem', color:'rgba(255,255,255,0.55)', lineHeight:1.65, maxWidth:560, fontFamily:"'DM Sans',sans-serif" }}>
          ...
          <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:14, padding:'0.5rem', boxShadow:'0 1px 4px rgba(0,0,0,0.04)', position:'sticky', top:24 }}>
            {CALCS.map(c => {
              ...
              return (
                ...
                >
                  <Icon size={14} color={isActive ? c.color : '#9ca3af'} style={{ flexShrink:0 }}/>
                  <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'0.8125rem', fontWeight: isActive ? 700 : 400, color: isActive ? c.color : '#6b7280', lineHeight:1.3 }}>{c.label}</span>
                </button>
              );
            })}
          </div>
