...
function LifeInsuranceLearn() {
  ...
  const stages = [
    ...
    { stage:'Retired, financially independent', need:'Low–None', color:'#6b7280', note:'If assets exceed liabilities and can support surviving spouse, life insurance may no longer be needed.' },
  ];
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
      {/* Type comparison */}
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>Term vs. Whole vs. Universal Life</h4>
            ...
            <div key={t.name} style={{ border:`1.5px solid ${t.color}30`, borderRadius:12, overflow:'hidden' }}>
              <div style={{ padding:'0.75rem 1rem', background:`${t.color}08`, borderBottom:`1px solid ${t.color}18`, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <span style={{ fontFamily:"'Playfair Display',serif", fontSize:'0.9375rem', fontWeight:700, color:NAVY }}>{t.name}</span>
                  <span style={{ padding:'2px 9px', background:`${t.color}18`, border:`1px solid ${t.color}35`, borderRadius:100, fontSize:'0.6875rem', fontWeight:700, color:t.color, fontFamily:"'DM Sans',sans-serif" }}>{t.badge}</span>
                </div>
                <div style={{ display:'flex', gap:'1rem', fontSize:'0.8125rem', fontFamily:"'DM Sans',sans-serif" }}>
                  <span style={{ color:'#9ca3af' }}>Cost: <strong style={{ color:NAVY }}>{t.cost}</strong></span>
                  <span style={{ color:'#9ca3af' }}>Duration: <strong style={{ color:NAVY }}>{t.coverage}</strong></span>
                </div>
              </div>
                ...
                <div>
                  {t.cons.map((c, i) => (
                    <div key={i} style={{ display:'flex', gap:7, alignItems:'flex-start', marginBottom:'0.375rem' }}>
                      ...
                      <span style={{ fontSize:'0.8125rem', color:'#374151', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{c}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ padding:'0.5rem 1rem 0.875rem', fontSize:'0.8125rem', color:'#374151', fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {/* Life stages */}
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>Coverage Need by Life Stage</h4>
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
          {stages.map((s, i) => (
            <div key={i} style={{ display:'flex', gap:'0.875rem', alignItems:'flex-start', padding:'0.75rem 0.875rem', background:'#f9fafb', borderRadius:10, border:'1px solid #f0f0f0' }}>
              <div style={{ width:6, height:6, borderRadius:'50%', background:s.color, flexShrink:0, marginTop:6 }}/>
              <div style={{ flex:1 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                  <span style={{ fontWeight:700, color:NAVY, fontSize:'0.875rem', fontFamily:"'DM Sans',sans-serif" }}>{s.stage}</span>
                  <span style={{ padding:'1px 8px', background:`${s.color}15`, borderRadius:100, fontSize:'0.6875rem', fontWeight:700, color:s.color, fontFamily:"'DM Sans',sans-serif" }}>{s.need} need</span>
                </div>
                <span style={{ fontSize:'0.8125rem', color:'#6b7280', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{s.note}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Premium table */}
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>Sample Term Life Premiums — $500K, 20-Year Term</h4>
            ...
            <thead>
              <tr style={{ borderBottom:'2px solid #e5e7eb' }}>
                {['Age','Health','Term','Coverage','Est. Monthly'].map(h => (
                  <th key={h} style={{ padding:'0.5rem 0.75rem', textAlign:'left', fontSize:'0.75rem', fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.05em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {premiumData.map((row, i) => (
                <tr key={i} style={{ borderBottom:'1px solid #f0f0f0', background: i%2===0?'#fafafa':'#fff' }}>
                  ...
                  <td style={{ padding:'0.625rem 0.75rem', color:'#374151' }}>{row.health}</td>
                  <td style={{ padding:'0.625rem 0.75rem', color:'#374151' }}>{row.term}</td>
                  <td style={{ padding:'0.625rem 0.75rem', color:'#374151' }}>{row.coverage}</td>
...
function HealthInsuranceLearn() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>HMO vs PPO vs HDHP</h4>
        <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
          {plans.map(p => (
            <div key={p.name} style={{ border:`1.5px solid ${p.color}30`, borderRadius:12, overflow:'hidden' }}>
              <div style={{ padding:'0.75rem 1rem', background:`${p.color}08`, borderBottom:`1px solid ${p.color}15`, display:'flex', alignItems:'center', gap:8 }}>
                <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:'0.9375rem', color:NAVY }}>{p.name}</span>
                <span style={{ fontSize:'0.8125rem', color:'#6b7280', fontFamily:"'DM Sans',sans-serif" }}>— {p.best}</span>
              </div>
              <div style={{ padding:'0.875rem 1rem', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 1rem' }}>
                <div>
                  {p.pros.map((x, i) => <div key={i} style={{ display:'flex', gap:7, marginBottom:'0.3rem' }}><CheckCircle2 size={12} color='#22c55e' style={{ flexShrink:0, marginTop:2 }}/><span style={{ fontSize:'0.8125rem', color:'#374151', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{x}</span></div>)}
                </div>
                <div>
                  {p.cons.map((x, i) => <div key={i} style={{ display:'flex', gap:7, marginBottom:'0.3rem' }}><XCircle size={12} color='#ef4444' style={{ flexShrink:0, marginTop:2 }}/><span style={{ fontSize:'0.8125rem', color:'#374151', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{x}</span></div>)}
                </div>
              </div>
            </div>
          ))}
      ...
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>Health Plan Anatomy — Key Terms</h4>
        {/* Visual flow */}
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1rem', padding:'1rem', background:'#f9fafb', borderRadius:12 }}>
          <div style={{ textAlign:'center', fontSize:'0.75rem', fontWeight:700, color:'#9ca3af', letterSpacing:'0.06em', textTransform:'uppercase', fontFamily:"'DM Sans',sans-serif", marginBottom:'0.5rem' }}>Your cost journey in a year</div>
          {[
            ...
            <div key={i} style={{ display:'flex', gap:'0.75rem', alignItems:'center' }}>
              <div style={{ width:`${s.pct}%`, maxWidth:120, height:28, background:s.color, borderRadius:7, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <span style={{ fontSize:'0.6875rem', fontWeight:700, color:'#fff', fontFamily:"'DM Sans',sans-serif" }}>Phase {i+1}</span>
              </div>
              <div>
                <div style={{ fontSize:'0.8125rem', fontWeight:700, color:NAVY, fontFamily:"'DM Sans',sans-serif" }}>{s.label}</div>
                <div style={{ fontSize:'0.75rem', color:'#9ca3af', fontFamily:"'DM Sans',sans-serif" }}>{s.sub}</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
          {terms.map((t, i) => (
            <div key={i} style={{ display:'flex', gap:'0.875rem', padding:'0.625rem 0.875rem', background:'#fff', border:'1px solid #e5e7eb', borderRadius:10 }}>
              <div style={{ width:6, height:6, borderRadius:'50%', background:t.color, flexShrink:0, marginTop:6 }}/>
              <div>
                <span style={{ fontWeight:700, color:NAVY, fontSize:'0.875rem', fontFamily:"'DM Sans',sans-serif" }}>{t.term}: </span>
                <span style={{ fontSize:'0.875rem', color:'#374151', lineHeight:1.65, fontFamily:"'DM Sans',sans-serif" }}>{t.def}</span>
              </div>
            </div>
          ))}
        </div>
...
function AutoInsuranceLearn() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
      <p style={{ fontSize:'0.875rem', color:'#6b7280', margin:'0 0 0.5rem', lineHeight:1.7, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {coverages.map(c => (
        <div key={c.name} style={{ border:`1.5px solid ${c.color}25`, borderRadius:12, overflow:'hidden' }}>
          <div style={{ padding:'0.75rem 1rem', background:`${c.color}08`, borderBottom:`1px solid ${c.color}18`, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
            <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:'0.9375rem', color:NAVY }}>{c.name}</span>
            <span style={{ padding:'2px 10px', background:`${c.color}15`, border:`1px solid ${c.color}30`, borderRadius:100, fontSize:'0.6875rem', fontWeight:700, color:c.color, fontFamily:"'DM Sans',sans-serif" }}>{c.required}</span>
          </div>
          <div style={{ padding:'0.75rem 1rem' }}>
            <p style={{ margin:'0 0 0.5rem', fontSize:'0.875rem', color:'#374151', lineHeight:1.7, fontFamily:"'DM Sans',sans-serif" }}>{c.desc}</p>
            <div style={{ display:'flex', gap:7, padding:'0.5rem 0.75rem', background:`${c.color}0a`, borderRadius:8 }}>
              ...
              <p style={{ margin:0, fontSize:'0.8rem', color:'#4b5563', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{c.tip}</p>
            </div>
          </div>
        </div>
      ))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.875rem' }}>
        <div style={{ border:'1.5px solid #22c55e30', borderRadius:12, overflow:'hidden' }}>
          <div style={{ padding:'0.625rem 0.875rem', background:'rgba(34,197,94,0.08)', borderBottom:'1px solid rgba(34,197,94,0.15)' }}>
            <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:'0.9375rem', color:NAVY }}>Typically Covered</span>
          </div>
          <div style={{ padding:'0.75rem 0.875rem' }}>
            {covered.map((x, i) => <div key={i} style={{ display:'flex', gap:7, marginBottom:'0.375rem' }}><CheckCircle2 size={12} color='#22c55e' style={{ flexShrink:0, marginTop:2 }}/><span style={{ fontSize:'0.8125rem', color:'#374151', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{x}</span></div>)}
          </div>
        </div>
        <div style={{ border:'1.5px solid #ef444430', borderRadius:12, overflow:'hidden' }}>
          <div style={{ padding:'0.625rem 0.875rem', background:'rgba(239,68,68,0.06)', borderBottom:'1px solid rgba(239,68,68,0.15)' }}>
            <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:'0.9375rem', color:NAVY }}>Not Covered</span>
          </div>
          <div style={{ padding:'0.75rem 0.875rem' }}>
            {notCovered.map((x, i) => <div key={i} style={{ display:'flex', gap:7, marginBottom:'0.375rem' }}><XCircle size={12} color='#ef4444' style={{ flexShrink:0, marginTop:2 }}/><span style={{ fontSize:'0.8125rem', color:'#374151', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{x}</span></div>)}
          </div>
        </div>
      </div>

      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>Replacement Cost vs Actual Cash Value (ACV)</h4>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.875rem' }}>
          {[
            ...
            <div key={t.type} style={{ padding:'1rem', background:`${t.color}08`, border:`1.5px solid ${t.color}25`, borderRadius:12 }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, color:NAVY, marginBottom:6, fontSize:'0.9375rem' }}>{t.type}</div>
              <p style={{ margin:'0 0 0.625rem', fontSize:'0.8125rem', color:'#374151', lineHeight:1.65, fontFamily:"'DM Sans',sans-serif" }}>{t.desc}</p>
              <div style={{ padding:'0.5rem 0.75rem', background:'rgba(255,255,255,0.7)', borderRadius:8, fontSize:'0.8rem', color:'#4b5563', fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>{t.example}</div>
            </div>
          ))}
        </div>
...
function LTCLearn() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
      <p style={{ fontSize:'0.875rem', color:'#6b7280', margin:0, lineHeight:1.7, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>National Average LTC Costs (2024)</h4>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontFamily:"'DM Sans',sans-serif", fontSize:'0.875rem' }}>
            <thead>
              <tr style={{ borderBottom:'2px solid #e5e7eb' }}>
            ...
            <tbody>
              {costs.map((row, i) => (
                <tr key={i} style={{ borderBottom:'1px solid #f0f0f0', background: i%2===0?'#fafafa':'#fff' }}>
                  ...
                  <td style={{ padding:'0.625rem 0.75rem', color:'#374151' }}>{row.daily}</td>
                  <td style={{ padding:'0.625rem 0.75rem', color:'#374151' }}>{row.monthly}</td>
      ...
      <div>
        <h4 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:NAVY, margin:'0 0 0.875rem' }}>When to Buy LTC Insurance</h4>
        <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
          {[
            { age:'Under 50', rec:'Too early', color:'#9ca3af', note:'Premiums are low but you\'ll pay them for decades. Most advisors suggest waiting.' },
            ...
            <div key={i} style={{ display:'flex', gap:'0.875rem', alignItems:'center', padding:'0.625rem 0.875rem', background:'#f9fafb', borderRadius:9, border:'1px solid #f0f0f0' }}>
              <div style={{ width:60, textAlign:'center' }}>
                <div style={{ fontWeight:800, color:NAVY, fontSize:'0.875rem', fontFamily:"'DM Sans',sans-serif" }}>{s.age}</div>
              </div>
              <div style={{ width:2, height:32, background:'#e5e7eb' }}/>
              <div style={{ flex:1 }}>
                <span style={{ padding:'2px 9px', background:`${s.color}15`, borderRadius:100, fontSize:'0.6875rem', fontWeight:700, color:s.color, fontFamily:"'DM Sans',sans-serif", marginBottom:4, display:'inline-block' }}>{s.rec}</span>
                <div style={{ fontSize:'0.8125rem', color:'#6b7280', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{s.note}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
        ...
        {[
          ...
          <div key={t.type} style={{ border:`1.5px solid ${t.color}30`, borderRadius:12, overflow:'hidden' }}>
            <div style={{ padding:'0.75rem 1rem', background:`${t.color}0c`, borderBottom:`1px solid ${t.color}18` }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:'0.875rem', color:NAVY, marginBottom:6 }}>{t.type}</div>
              <div style={{ display:'flex', flexDirection:'column', gap:3, fontSize:'0.75rem', fontFamily:"'DM Sans',sans-serif" }}>
                <span style={{ color:'#9ca3af' }}>Duration: <strong style={{ color:NAVY }}>{t.duration}</strong></span>
                <span style={{ color:'#9ca3af' }}>Wait period: <strong style={{ color:NAVY }}>{t.wait}</strong></span>
                <span style={{ color:'#9ca3af' }}>Typical coverage: <strong style={{ color:t.color }}>{t.covers}</strong></span>
              </div>
            </div>
            <div style={{ padding:'0.75rem 1rem' }}>
              {t.points.map((p, i) => <div key={i} style={{ display:'flex', gap:7, marginBottom:'0.35rem' }}><div style={{ width:5, height:5, borderRadius:'50%', background:t.color, flexShrink:0, marginTop:5 }}/><span style={{ fontSize:'0.8125rem', color:'#374151', lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{p}</span></div>)}
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding:'0.875rem 1rem', background:'rgba(0,180,198,0.07)', border:'1px solid rgba(0,180,198,0.2)', borderRadius:12 }}>
        <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, color:NAVY, marginBottom:6, fontSize:'0.9375rem' }}>Own-Occupation vs Any-Occupation — The Most Important Distinction</div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.75rem', fontSize:'0.8125rem', fontFamily:"'DM Sans',sans-serif" }}>
          <div style={{ padding:'0.625rem 0.75rem', background:'rgba(34,197,94,0.08)', borderRadius:9, color:'#374151' }}>
          ...
          <div style={{ padding:'0.625rem 0.75rem', background:'rgba(239,68,68,0.07)', borderRadius:9, color:'#374151' }}>
...
function DIMECalc() {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:'1rem' }}>
        <label style={{ display:'block', fontSize:'0.8125rem', fontWeight:600, color:NAVY, marginBottom:'0.375rem', fontFamily:"'DM Sans',sans-serif" }}>Income Replacement Years — {incomeYears} years</label>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.6875rem', color:'#9ca3af', fontFamily:"'DM Sans',sans-serif", marginTop:2 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1rem' }}>
        {items.map(it => (
          <div key={it.label} style={{ display:'flex', alignItems:'center', gap:'0.875rem', padding:'0.625rem 0.875rem', background:'#f9fafb', borderRadius:9 }}>
            <div style={{ width:10, height:10, borderRadius:3, background:it.color, flexShrink:0 }}/>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:'0.8125rem', fontWeight:700, color:NAVY, fontFamily:"'DM Sans',sans-serif" }}>{it.label}</div>
              <div style={{ fontSize:'0.75rem', color:'#9ca3af', fontFamily:"'DM Sans',sans-serif" }}>{it.desc}</div>
            </div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:it.color }}>{fmt(it.value)}</div>
          </div>
        ))}
        {existing > 0 && (
          <div style={{ display:'flex', alignItems:'center', gap:'0.875rem', padding:'0.625rem 0.875rem', background:'#f0fdff', borderRadius:9, border:'1px solid rgba(0,180,198,0.2)' }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontSize:'0.8125rem', fontWeight:700, color:NAVY, fontFamily:"'DM Sans',sans-serif" }}>Minus existing coverage</div>
            </div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:700, color:'#22c55e' }}>−{fmt(existing)}</div>
          </div>
        )}
      </div>
        ...
        {RESOURCES.map(cat => (
          <div key={cat.category}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:'0.75rem' }}>
              ...
              <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, color:NAVY, fontSize:'1rem' }}>{cat.category}</span>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:'0.625rem' }}>
              {cat.providers.map(p => (
                <div key={p.name} style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:12, padding:'0.875rem 1.125rem', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
                    <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, color:NAVY, fontSize:'0.9375rem' }}>{p.name}</span>
                    <span style={{ padding:'2px 9px', background:`${cat.color}15`, border:`1px solid ${cat.color}30`, borderRadius:100, fontSize:'0.6875rem', fontWeight:700, color:cat.color, fontFamily:"'DM Sans',sans-serif" }}>{p.badge}</span>
                  </div>
                  <p style={{ margin:0, fontSize:'0.875rem', color:'#374151', lineHeight:1.65, fontFamily:"'DM Sans',sans-serif" }}>{p.desc}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    <div style={{ minHeight:'100vh', background:BG, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ background:NAVY, padding:'2rem 2.5rem 0' }}>
        <div style={{ fontSize:'0.75rem', color:'rgba(255,255,255,0.35)', marginBottom:'1rem', display:'flex', alignItems:'center', gap:6 }}>
          ...
          <span style={{ fontFamily:"'DM Sans',sans-serif" }}>Insurance Planning</span>
        </div>
      ...
      <div style={{ padding:'2rem 2.5rem', maxWidth:860, margin:'0 auto' }}>
        ...
        <div onClick={() => navigate('/fun/estate')} style={{ marginTop:'2rem', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1rem 1.25rem', background:NAVY, borderRadius:12, cursor:'pointer', transition:'opacity 0.15s' }}
          ...
          <div>
            ...
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1rem', fontWeight:600, color:'#fff' }}>Estate Planning & Wills</div>
          </div>
